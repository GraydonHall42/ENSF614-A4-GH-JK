...
public:
  ...
  MyArray(const MyArray& source); // copy 
  MyArray& operator =(const MyArray& rhs); // overload the assignment operator, so that the copy constructor is called when we use it
  ~MyArray();
